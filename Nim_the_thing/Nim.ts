...
// Setup
let r0: number = randomInt(2, 11);
let r1: number = randomInt(2, 11);
let r2: number = randomInt(2, 11);
let r3: number = randomInt(2, 11);
const validpiles: number[] = [1, 2, 3, 4];
let correctinput: boolean = false;
...
function _playerturn(): void {
    ...
    while (!correctinput) {
        ...
        if (pilenumberInput === "1" || pilenumberInput === "2" || pilenumberInput === "3" || pilenumberInput === "4") {
            let pilenumber: number = parseInt(pilenumberInput);
            let lampcount: number;
            while (true) {
...
function _ispileemthyorremove(pile: number, currentpilecount: number, count: number): number {
...
function _isPileEmthyOrRemoveEasy(pile: number, currentpilecount: number, count: number): number {
    currentpilecount -= count;
    if (currentpilecount === 0) {
        const index = validpiles.indexOf(pile);
        if (index > -1) {
